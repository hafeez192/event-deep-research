...
async def test_langfuse_loading():
    """Test that langfuse loading doesn't block."""
    try:
        from src.graph import get_langfuse_handler, get_langfuse_client
        
        # Test langfuse client
        client = get_langfuse_client()
        print("✓ Langfuse client creation works")
        
        # Test langfuse handler
        handler = get_langfuse_handler()
        print("✓ Langfuse handler creation works")
        return True
    except Exception as e:
        print(f"✗ Langfuse loading failed: {e}")
        return False
